...
시소러스 에디터는 용어의 계층적 관계를 정의하고 관리하는 도구입니다. 대표 명칭(Primary Term)과 하위 서브 명칭(Sub Terms)을 생성하여 용어 간의 관계를 정의하고, 정규표현식을 활용한 패턴 매칭 기능을 제공합니다.

[메뉴바](../program/menu-bar.md#setting) 에서 실행할 수 있습니다.
...
**[화면 캡쳐 - Thesaurus Editor 메인 화면]**
![차트 선택 화면](/img/screenshots/thesaurus-editor.png)
...
| 버튼명           | 동작                                                                                                                       |
| :--------------- | :------------------------------------------------------------------------------------------------------------------------- |
| **Open file**    | 기존 시소러스 파일을 불러옵니다. KM, VOSViewer, VantagePoint 형식을 지원하며, 인코딩을 자동으로 감지합니다.                |
...
Thesaurus Editor는 다음 3가지 파일 형식을 지원합니다:
...
1. **Open file** 버튼으로 KM 형식 파일 로드 (.the, .tsv, .json)
2. 필요한 경우 내용 편집
3. **Save as** 클릭
