...
:::info
메뉴 바에서는 KM+의 핵심 기능을 카테고리별로 제공합니다.
:::
...
* **홈 (Home):** 프로젝트 생성, 프로젝트 열기 및 저장, 프로그램 종료 등 파일 및 프로젝트 관리 기능을 수행합니다.
* **전처리 (Pre-processing):** 프로젝트 합집합 및 교집합 생성, 영문/한글 자연어 처리, 필드 바인딩 등의 기능을 제공합니다.
* **분석 (Analysis):** 2D 행렬 생성, 멀티 리스트 생성, 데이터 반출(export) 기능을 포함합니다.
* **차트 (Chart):** 리스트를 간단한 그래프로 구현합니다.
* **설정 (Settings):** 보기 설정, 임포트 필터 뷰어(Import Filter Viewer), 시소러스 에디터(Thesaurus Editor) 등을 포함합니다.
* **도움말 (Help):** 매뉴얼, 프로그램 정보 등을 확인할 수 있습니다.
...
![pre-processing-new dataset](/img/screenshots/menubar-newdataset.png)
...
|                          | Summary Sheet | 클릭하여 선택한 프로젝트의 **Summary Sheet**를 메인 작업 영역에 엽니다.  |
